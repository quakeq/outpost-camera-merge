...
def _packet_bytes(camera_id: str = "phone_a", seq: int = 1, t_ms: int = 1000) -> bytes:
    landmarks = [
...
def test_parse_packet_round_trip():
    data = _packet_bytes("phone_b", seq=42, t_ms=9999)
    frame = _parse_packet(data, "192.168.50.12")
    assert frame.camera_id == "phone_b"
    assert frame.seq == 42
...
def test_parse_packet_missing_key_raises():
    data = json.dumps({"camera_id": "phone_a", "seq": 1}).encode("utf-8")
    try:
...
def test_receiver_continues_after_bad_packet(monkeypatch, free_udp_port):
    ...
    try:
        ...
        sock.sendto(_packet_bytes("phone_a", seq=1, t_ms=1000), ("127.0.0.1", free_udp_port))
        # Wait for good packet to land
        deadline = time.time() + 2.0
        while time.time() < deadline:
            if store.nearest_all(1000)["phone_a"] is not None:
                break
            time.sleep(0.02)
        assert store.nearest_all(1000)["phone_a"] is not None
    finally:
        sock.close()
        stop.set()
